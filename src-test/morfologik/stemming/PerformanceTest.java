...
public class PerformanceTest {
	...
	int i = 0;
	for (ByteBuffer sequence : dict.fsa) {
	    testWords[i] = new String(
		    sequence.array(), 0, sequence.remaining(), 
		    dict.metadata.encoding);
	    testWords[i] = testWords[i].substring(
		    0, testWords[i].indexOf(dict.metadata.separator));
	    i++;
     ...
     * using String.getBytes and CharsetEncoder (which String.getBytes uses internally
     * in SUN's JDK).  
     */
    @Ignore
    @Test
