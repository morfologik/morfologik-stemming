...
public final class FSADumpTool extends Tool {
	...
	dataOnly = line.hasOption(
		SharedOptions.dataOnly.getOpt());
	...
	if (hasMetadata(dictionaryFile)) {
		...
		printExtra("Dictionary's charset is not supported "
	    ...
	    printExtra("Warning: FSA automaton without metadata file.");
	}
	...
	if (dictionary != null) {
	    ...
	    for (Map.Entry<String, String> e : dictionary.metadata.metadata.entrySet())
	    {
		printExtra(String.format("%-27s : %s", e.getKey(), e.getValue()));
	    }
	    printExtra("");
	}
